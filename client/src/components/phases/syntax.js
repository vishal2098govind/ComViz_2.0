...
function Syntax(props) {
  ...
  let ab = {
    abrv: [
      'Expression (Var-Assignment expressions or logical expressions )',
      'Comparison Expression',
      'Arithmetic  Expression',
      'Identifier',
      '=',
      'Keyword to indicate variable declaration',
      'Term',
      'Factor',
      'Power',
      'Atom',
      ...
      'Paranthesis',
      'Keyword to indicate logical AND, OR & NOT',
      'Comprison operators',
    ],
    term: [
      'E',
    ...
    },
    {
      getProps: (state, rowInfo) => {
        if (rowInfo && rowInfo.row) {
          return {
            style: {
              background: rowInfo.row.int ? 'red' : null,
            },
          };
        } else {
          return {};
    ...
    switch (step) {
      case 0:
        return (
          <div>
            {parserType != 'topDown' ? (
              <div style={{ position: 'fixed', right: '0', top: '10%' }}>
                <Button
                  variant='contained'
                  color='default'
            ...
            <div style={{ display: 'inline-block', marginTop: '-12px' }}>
              <div
                style={{
                  ...
                  marginBottom: `${parserType != 'topDown' ? '90px' : '20px'}`,
                }}
              >
                <ReactTable
                  data={
                    parserType == 'topDown'
                      ? topDowngrammarTableData
                      : bottomUpgrammarTableData
                  }
      ...
      case 1:
        return (
          <div style={{ display: 'inline-block' }}>
            {parserType == 'topDown' ? (
              <ReactTable parserType={parserType} />
            ) : (
  ...
  return (
    <div className={classes.root1}>
      ...
      >
        <spam style={{ color: '#FEFFFF' }}>{title[step].first}</spam>{' '}
        {title[step].second}
      </Typography>
      {syntaxStep()}
      <div style={{ position: 'fixed', left: '32%', bottom: '0%' }}>
        ...
        <Button variant='contained' color='default' className={classes.button1}>
          <Select
          ...
          >
            <MenuItem value={'topDown'}>Top-Down Parser</MenuItem>
      ...
      {(errorMessageTopDown && parserType == 'topDown') ||
      (errorMessageBottomUp && parserType == 'bottomUp') ? (
        <ErrorBar
          text={`${
            errorMessageTopDown && parserType == 'topDown'
              ? errorMessageTopDown
